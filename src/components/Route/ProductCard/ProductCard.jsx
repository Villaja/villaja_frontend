...
const ProductCard = ({ data, isEvent, catIndex, itemDisplay = true }) => {
  ...
  return (
    <>
      <div className={`${catIndex ? "pc-container-mobile" : ""} ${itemDisplay ? "" : "single-item-container-mobile"} pc-container w-full h-[300px] sm:h-[330px] bg-white rounded-lg sm:shadow-sm pt-3 relative cursor-pointer !shadow-[0_24px_36px_0px_rgba(52,87,140,0.12)]`}>
          ...
          <div className="py-2 px-3 flex items-center justify-between">
            <div className="flex">
              <h5 className="pr-1 font-semibold text-gray-800 text-[1.1rem] min-[500px]:text-[1.2rem]">
                {'₦' + (data.discountPrice === 0 || data.discountPrice === null ? data.originalPrice?.toLocaleString() : data.discountPrice?.toLocaleString())}
              </h5>
              <h4 className="text-[0.55rem] line-through block text-red-400">
                {data.discountPrice !== 0 && data.discountPrice !== null ? '₦' + (data.originalPrice?.toLocaleString() || '') : null}
              </h4>
            </div>
